...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    cache=(x,numer,denom,x_hat,sample_mean,sample_var,gamma,beta,eps)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
  elif mode == 'test':
...
def batchnorm_backward(dout, cache):
  ...
  #############################################################################
  x,numer,denom,x_hat,sample_mean,sample_var,gamma,beta,eps=cache
  N=x.shape[0]
  ...
  dnorm=dout*gamma
  dnorm2=np.sum(-(numer*dnorm)/(denom**2),axis=0)
  dsqrt=dnorm2/(2*denom)
  dsample_var=(2*x-2*sample_mean)*dsqrt
  dsample_mean=np.sum(-dnorm/denom,axis=0)
...
def batchnorm_backward_alt(dout, cache):
  ...
  dx=1.0/(N*denom)*gamma*(N*dout-dbeta-numer*dgamma/denom)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def max_pool_forward_naive(x, pool_param):
  ...
  for i in range(Hout):
    for j in range(Wout):
      out[:,:,i,j]=x[:,:,i*PS:i*PS+PH,j*PS:j*PS+PW].max(axis=3).max(axis=2)
...
def max_pool_backward_naive(dout, cache):
  ...
  for i in range(N):
    for j in range(C):
      for k in range(Hout):
        for l in range(Wout):
          dx_index=np.unravel_index(x[i,j,k*PS:k*PS+PH,l*PS:l*PS+PW].argmax(),(PH,PW))
          dx[i,j,dx_index[0]+k*PS,dx_index[1]+l*PS] += dout[i,j,k,l]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    running_mean = momentum * running_mean + (1 - momentum) * sample_mean.reshape(-1)
    running_var = momentum * running_var + (1 - momentum) * sample_var.reshape(-1)
...
def spatial_batchnorm_backward(dout, cache):
  ...
  dx=1.0/(N*H*W*denom)*gamma*(N*H*W*dout-dbeta.reshape(1,C,1,1)
    -numer*dgamma.reshape(1,C,1,1)/denom)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def softmax_loss(x, y):
  ...
  # probs=np.exp(x) ## the difference is just a scaling factor in the denominator
